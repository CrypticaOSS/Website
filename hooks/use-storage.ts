    ...
    try {
      ...
      switch (dbConfig.type) {
        case 'supabase': {
          ...
          const response = await fetch(`${formattedUrl}/items?key=eq.${encodeURIComponent(key)}`, {
            method: 'GET',
            headers
          });
        ...
        case 'firebase': {
          const url = new URL(`${formattedUrl}/items/${encodeURIComponent(key)}.json`);
          if (dbConfig.apiKey) {
        ...
        default: {
          ...
          const response = await fetch(`${formattedUrl}/items/${encodeURIComponent(key)}`, {
            method: 'GET',
            headers
          });
    ...
    try {
      ...
      switch (dbConfig.type) {
        case 'supabase': {
          ...
          const checkResponse = await fetch(`${formattedUrl}/items?key=eq.${encodeURIComponent(key)}`, {
            method: 'GET',
            headers
          });
          ...
          if (checkResponse.ok) {
            ...
            if (existingData && existingData.length > 0) {
              ...
              await fetch(`${formattedUrl}/items?key=eq.${encodeURIComponent(key)}`, {
                method: 'PATCH',
                headers,
